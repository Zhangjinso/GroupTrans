...
import numpy as np
import pickle
from jrdb_train_ours import get_person_traj_group_list,model_config,load_person_appearance
from tqdm import tqdm
from graph import graph_propagation
from group_eval import group_eval
from oursmodel import filter_impossible_edges,OverallModel1
...
def plot_traj_pair(persona, personb):
    #     return 3 plot including time 1/3, 2/3, 1
    ...
    with torch.no_grad():
        for id, scene in enumerate(val_set):
            ...
            for p in all_ps:
                ...
                if len(all[p]["group"]) != 0:
                    gp_label.append(all[p]["group"][0])
                else:
                    gp_label.append(nogp_id)
                    nogp_id -=1
            input_feat = []
            frame_mask = []
            input_skeletonft = []
            ...
            tot_person += input_feat.shape[0]
            eoi = []
            label = []
            for i in range(1,len(sampled_ps)):
                for j in range(i):
                    eoi.append([i,j])
                    label.append(gp_label[i] == gp_label[j])
                    
            eoi = np.array(eoi,np.int).T
            print("before filter",eoi.shape[1])
            eoi, emsk = filter_impossible_edges(np.transpose(input_feat,[0,2,1]),eoi,frame_mask,MIN_DIS_THR,min_time_overlap=config_param.TEST_MIN_TIMEOVERLAP_THR)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--taskname', type=str, default='oursjrdb')
    # dataset
    ...
    if cfg.refresh_ana == 1:
        ...
        val_set = []
        val_set_name = []
        with open(f"./dataset/jrdb_annotation/group_{ds_split}.txt") as f:
            print("start gen pkl")
            tmp_all = f.readlines()
            for i in tqdm(range(0, len(tmp_all))):
                ...
                # val_set_name.append(os.path.basename(os.path.dirname(tmp_all[i].split(" ")[0].replace("\n", ""))))
    ...
    if cfg.loading_tracjory_net != "":
        ...
        #print(cfg.loading_tracjory_net)
    for id, scene in enumerate(val_set):
